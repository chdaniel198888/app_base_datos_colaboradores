...
export function useSearch(query: string, options: UseSearchOptions = {}) {
  ...
  useEffect(() => {
    ...
    // Si la query es muy corta y no hay filtros, limpiar resultados
    if (query.length < 2 && !options.filters?.centro_costo && !options.filters?.marca && !options.filters?.area) {
      setResults([]);
      setSource(null);
      return;
    }
    ...
    if (currentSearch === lastQueryRef.current && results.length > 0) {
      return;
    }
