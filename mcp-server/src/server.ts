...
async function buscarColaborador(query: string): Promise<Colaborador[]> {
  ...
  return records.map((record) => ({
    ...
    local: record.get('Local') as string,
    direccion: record.get('Dirección') as string,
    celular: record.get('Celular') as string,
    email: record.get('Correo Electronico') as string,
    fechaIngreso: record.get('Fecha de Ingreso') as string,
    fechaNacimiento: record.get('Fecha de nacimiento') as string,
    estado: record.get('Estado') as string,
    reportaA: record.get('Reporta a') as string,
    foto: record.get('Foto') as any,
  }));
}
